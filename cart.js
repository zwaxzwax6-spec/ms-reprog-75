...
// === CUSTOM SELECT (dropdown stylé) ===
function initCustomSelects() {
  document.querySelectorAll('.select-custom').forEach(select => {
    const trigger = select.querySelector('.select-custom-trigger');
    const valueEl = select.querySelector('.select-custom-value');
    const dropdown = select.querySelector('.select-custom-dropdown');
    const hiddenInput = select.querySelector('input[type="hidden"]');
    const placeholder = valueEl.dataset.placeholder || '— Sélectionner —';

    // Toggle dropdown
    trigger.addEventListener('click', (e) => {
      e.stopPropagation();
      // Fermer les autres dropdowns ouverts
      document.querySelectorAll('.select-custom.open').forEach(s => {
    ...
    // Click sur option
    dropdown.querySelectorAll('.select-custom-option').forEach(option => {
      option.addEventListener('click', () => {
        ...
        dropdown.querySelectorAll('.select-custom-option').forEach(o => o.classList.remove('selected'));
        option.classList.add('selected');

        select.classList.remove('open');

        // Trigger change event sur l'input hidden (si form validation)
        if (hiddenInput) {
  ...
  // Click outside → fermer tous les dropdowns
